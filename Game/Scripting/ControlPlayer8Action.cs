...
namespace black_friday.Game.Scripting{
    class ControlPlayer8Action : Action{
        KeyboardService keyboardService = new KeyboardService();
        public ControlPlayer8Action(){
        }
        public void Execute(Scene scene){
            ...
            // punch
            if (keyboardService.IsKeyDown("n"))
            {
                isPunching = true;
            }
            // up
            ...
            // left
            if (keyboardService.IsKeyDown("m"))
            {
                vx = -1;
            }
            // down
            ...
            foreach(Actor actor in players){
                ...
                if(player.GetPlayerID() == 8 && player.GetIsAlive()){
