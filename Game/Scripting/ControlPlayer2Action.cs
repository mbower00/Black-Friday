...
namespace black_friday.Game.Scripting{
    class ControlPlayer2Action : Action{
        ...
        public void Execute(Scene scene){
            ...
            // punch
            if (keyboardService.IsKeyDown("u"))
            {
                isPunching = true;
            }
            // up
            if (keyboardService.IsKeyDown("y"))
            {
                vy = -1;
            }
            // left
            if (keyboardService.IsKeyDown("g"))
            {
                vx = -1;
            }
            // down
            if (keyboardService.IsKeyDown("h"))
            {
                vy = 1;
            }
            // right
            ...
            foreach(Actor actor in players){
                ...
                if(player.GetPlayerID() == 2 && player.GetIsAlive()){
