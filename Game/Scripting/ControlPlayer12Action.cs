...
namespace black_friday.Game.Scripting{
    class ControlPlayer12Action : Action{
        KeyboardService keyboardService = new KeyboardService();
        public ControlPlayer12Action(){
        }
        public void Execute(Scene scene){
            ...
            // up
            if (keyboardService.IsKeyDown("backspace"))
            {
                vy = -1;
            }
            // left
            ...
            // down
            if (keyboardService.IsKeyDown("enter"))
            {
                vy = 1;
            }
            // right
            ...
            foreach(Actor actor in players){
                ...
                if(player.GetPlayerID() == 12 && player.GetIsAlive()){
