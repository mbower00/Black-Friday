...
namespace black_friday.Game.Scripting{
    class ControlPlayer11Action : Action{
        KeyboardService keyboardService = new KeyboardService();
        public ControlPlayer11Action(){
        }
        public void Execute(Scene scene){
            ...
            // punch
            if (keyboardService.IsKeyDown("f7"))
            {
                isPunching = true;
            }
            // up
            if (keyboardService.IsKeyDown("f8"))
            {
                vy = -1;
            }
            // left
            if (keyboardService.IsKeyDown("t6"))
            {
                vx = -1;
            }
            // down
            if (keyboardService.IsKeyDown("t7"))
            {
                vy = 1;
            }
            // right
            if (keyboardService.IsKeyDown("t8"))
            {
                vx = 1;
            }
            ...
            foreach(Actor actor in players){
                ...
                if(player.GetPlayerID() == 11 && player.GetIsAlive()){
