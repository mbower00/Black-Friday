...
namespace black_friday.Game.Scripting{
    class ControlPlayer9Action : Action{
        KeyboardService keyboardService = new KeyboardService();
        public ControlPlayer9Action(){
        }
        public void Execute(Scene scene){
            ...
            // punch
            if (keyboardService.IsKeyDown("f4"))
            {
                isPunching = true;
            }
            // up
            if (keyboardService.IsKeyDown("f3"))
            {
                vy = -1;
            }
            // left
            if (keyboardService.IsKeyDown("t3"))
            {
                vx = -1;
            }
            // down
            if (keyboardService.IsKeyDown("t4"))
            {
                vy = 1;
            }
            // right
            if (keyboardService.IsKeyDown("t5"))
            {
                vx = 1;
            }
            ...
            foreach(Actor actor in players){
                ...
                if(player.GetPlayerID() == 9 && player.GetIsAlive()){
